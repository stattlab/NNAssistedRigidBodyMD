...
inline void AABBTree::init(unsigned int N)
    {
    //std::cout<< "AABBTree::init "<< std::endl;
    // clear the nodes
    m_num_nodes = 0;
...
inline unsigned int AABBTree::buildNode(AABB *aabbs,
    ...
    Point my_radius = my_aabb.getUpper() - my_aabb.getLower();
    ...
    if (len == 2)
        ...
        if (my_radius.getX() > my_radius.getY() && my_radius.getX() > my_radius.getZ())
            {
            // split on x direction
            for (unsigned int i = 0; i < start_right; i++)
                {
                if (aabbs[start+i].getPosition().getX() < my_aabb.getPosition().getX())
                    {
        ...
        else if (my_radius.getY() > my_radius.getZ())
            {
            // split on y direction
            for (unsigned int i = 0; i < start_right; i++)
                {
                if (aabbs[start+i].getPosition().getY() < my_aabb.getPosition().getY())
                    {
                    // if on the left side, everything is happy, just continue on
                    }
                else
            ...
            for (unsigned int i = 0; i < start_right; i++)
                {
                if (aabbs[start+i].getPosition().getZ() < my_aabb.getPosition().getZ())
                    {
                    // if on the left side, everything is happy, just continue on
                    }
                else
